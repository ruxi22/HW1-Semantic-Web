...
function RecommendationsBySkillAndCuisinePage() {
  ...
  const fetchRecommendations = async (userId) => {
    try {
      setLoading(true);
      const response = await fetch(`http://localhost:8091/api/recommendations/skill-cuisine?userId=${userId}`);
      if (!response.ok) {
  ...
  return (
    <div className="page-container">
      ...
      {selectedUserInfo && (
        <div style={{marginBottom: '20px', padding: '15px', backgroundColor: '#fffde7', borderRadius: '8px', borderLeft: '5px solid #ffc107'}}>
          ...
          <p><strong>Preferred Cuisine:</strong> <span style={{backgroundColor: '#2196F3', color: 'white', padding: '4px 8px', borderRadius: '4px'}}>{selectedUserInfo.preferredCuisineType}</span></p>
        </div>
      )}

      <p className="subtitle">Perfect matches for {selectedUserInfo?.name} ({selectedUserInfo?.cookingSkillLevel} skill, {selectedUserInfo?.preferredCuisineType} cuisine) - {recipes.length} recipes found</p>

      <div className="recipes-grid">
        {recipes.length > 0 ? (
          recipes.map((recipe) => (
            <div key={recipe.id} className="recipe-card perfect-match">
              <div className="recipe-header">
                <span className="recipe-id">{recipe.id}</span>
                <span className="badge perfect"> Perfect Match</span>
              </div>
          ...
          <p>No perfect matches found for this user. Check out recommendations by skill level.</p>
        )}
      </div>
    </div>
  );
...
export default RecommendationsBySkillAndCuisinePage;
